...
import { useGetEngDataByPageQuery } from '../../featues/pageWiseEnglishData';
...
const PageWiseEngData: React.FC = () => {

    // state for tracking the page of the QuranPak
    const [page, setPage] = useState(1);

    // destructured the data loading and error from the hook
    const { data, isLoading, isError, isFetching } = useGetEngDataByPageQuery(page.toString());
                            ...
                            <div className="px-4 mx-auto max-w-screen-2xl py-8 mt-10 bg-neutral-400 rounded-lg">
                                ...
                                <div className="py-5 mt-2  h-96   bottom-0 top-0  overflow-y-scroll   rounded-lg bg-white px-3 cursor-pointe" >
                                    ...
                                    {data?.data.ayahs.map((item, i) => (
                                        ...
                                        <div key={i}>
                                            <h1 className="text-2xl text-start flex  items-center flex-wrap gap-2 p-3 hover:rounded-lg hover:cursor-pointer hover:text-white text-black hover:bg-stone-600  font-bold pb-4 font-serif">
                                ...
                                <div className='flex flex-row flex-wrap gap-4 my-4'>
                                    <button className='bg-white flex items-center gap-2 hover:bg-gray-100 text-black px-5 py-2' onClick={nextPage}>Next {page}(604)<FaArrowRightLong /></button>
...
export default PageWiseEngData;
